...
class PerfectPlayer < Player
  ...
  def minimax(board, depth, alpha, beta, is_maximizing)
    ...
    @node_count += 1
    if @debug && @node_count % 200_000 == 0
      elapsed = Time.now - @start_time
      nps = elapsed > 0 ? (@node_count / elapsed).to_i : 0
      print "\rThinking... Nodes: #{@node_count} (NPS: #{nps})"
    end
...
class Perfect2Player < Player
  ...
  def initialize(disc, name, debug = false)
    ...
    @tt = {}
    @start_time = Time.now
    @tt_max_size = 430_000_000 # 430 million entries for approx. 40GB
  end
  ...
  def get_move(board)
    ...
    if show_debug
      ...
      sorted_cols.each do |col|
        c_idx = col - 1
        move_bit = (mask & (0x7F << (c_idx * 7))) + (1 << (c_idx * 7))
  ...
  def solve(position, mask, depth, alpha, beta)
    ...
    [3, 2, 4, 1, 5, 0, 6].each do |c_idx|
      move_bit = (mask & (0x7F << (c_idx * 7))) + (1 << (c_idx * 7))
      next if (move_bit & (0x40 << (c_idx * 7))) != 0
  ...
  def check_win_bitboard(pos)
    m = pos & (pos >> 7)
    return true if (m & (m >> 14)) != 0
    m = pos & (pos >> 1)
    return true if (m & (m >> 2)) != 0
    m = pos & (pos >> 8)
    return true if (m & (m >> 16)) != 0
    m = pos & (pos >> 6)
    return true if (m & (m >> 12)) != 0
    false
  end
  ...
  def mirror(b)
    ...
    c4 = (b >> 28) & 0x3F
    c3 = (b >> 21) & 0x3F
    (c6) | (c5 << 7) | (c4 << 14) | (c3 << 21) | (c2 << 28) | (c1 << 35) | (c0 << 42)
  end
